export default function LayerControls({ layers, onToggle }) {
  ...
  return (
    <div className="absolute top-4 left-4 z-10 bg-gray-900/90 backdrop-blur-sm rounded-xl p-4 shadow-2xl border border-gray-700/50 min-w-[180px]">
      <h2 className="text-white text-[10px] font-semibold uppercase tracking-widest mb-3 opacity-70">
        Layers
      </h2>
        ...
        {controls.map(({ id, label, color }) => (
          <label key={id} className="flex items-center gap-3 cursor-pointer group">
            <div className="relative flex-shrink-0">
              ...
              />
              <div
                className="w-9 h-5 rounded-full transition-colors duration-200 peer-checked:opacity-100 opacity-30"
